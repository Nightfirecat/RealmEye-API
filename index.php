...
if (!preg_match('/^[a-z]{1,10}$/i', $player) && !preg_match('/^[a-z0-9]{11}$/i', $player)) {
...
function echo_json_and_exit($output_array) {
	...
	if (isset($_GET['pretty'])) {
		$output = json_encode($output_array, JSON_PRETTY_PRINT | $default_options);
	} else {
		$output = json_encode($output_array, $default_options);
	}
